...
export default function Home() {
  ...
  // Helper to get storage key for reactions (user-specific)
  const getReactionsStorageKey = () => `userReactions_${currentUser?.username || 'anonymous'}`;
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        // NEW: Load cached user reactions from localStorage first for instant display
        const cachedReactions = JSON.parse(localStorage.getItem(getReactionsStorageKey()) || '{}');
        setUserReactions(cachedReactions);
        ...
        if (cachedArticles && cacheTimestamp) {
          ...
          // Fetch reactions for cached articles (sync with server)
          if (savedToken) {
        ...
        setArticles(data);
        // Fetch reactions for articles (sync with server)
        if (savedToken) {
  ...
  // Fetch user's reactions for articles (updated to sync localStorage)
  const fetchReactionsForArticles = async (articlesList, authToken) => {
    try {
      ...
      // NEW: Cache the fetched reactions to localStorage for persistence on refresh
      localStorage.setItem(getReactionsStorageKey(), JSON.stringify(reactions));
    } catch (err) {
  ...
  // Like Button - Database backed (updated to sync localStorage)
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUserReactions((prev) => {
          ...
          // NEW: Update localStorage with new reactions
          localStorage.setItem(getReactionsStorageKey(), JSON.stringify(newReactions));
          return newReactions;
        });
        // Update counts
        if (data.count) {
  ...
  // Dislike Button - Database backed (updated to sync localStorage)
  ...
  // Bookmark Button Logic (local storage) - unchanged, but note: using idx is fragile if articles reorder; consider using article.link instead
